...
if __name__ == '__main__':
    ...
    for league in ['nba']:
        for schedule in ['planned', 'actual']:
            if schedule == 'actual':
                df = load_data(league)
            else:
                file_dir = os.getcwd()
                ...
                df.loc[(df['home'] == 'Dallas Mavericks') & (
                        df['visitor'] == 'Detroit Pistons') & (
                                    df['game_date'] == datetime.datetime(2021, 2, 17)
                            ), 'game_date'] = datetime.datetime(2021, 4, 21)

                df.loc[(df['home'] == 'Charlotte Hornets') & (
                        df['visitor'] == 'Chicago Bulls') & (
                                    df['game_date'] == datetime.datetime(2021, 2, 17)
                            ), 'game_date'] = datetime.datetime(2021, 5, 6)
            ...
            df_stats.to_csv(f'./results/{league}_{schedule}_schedule_rules.csv', index=False, encoding='utf-8 sig')
