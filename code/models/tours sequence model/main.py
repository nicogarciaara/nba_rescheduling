...
from ttp_model import TourSequenceModel, get_disruptions_and_non_disruptions
import numpy as np
import warnings
import datetime
import cplex
...
cwd = os.getcwd()
if "models" in cwd:
    eda_wd = cwd.replace('models\\tours sequence model', 'eda')
else:
    eda_wd = cwd + '\\code\\eda'
sys.path.insert(0, eda_wd)
from scheduling_rules import *
base_cdw = "C:/Users/HP/Documents/Sports Analytics/Re Scheduling/code/models/tours sequence model/"
...
if __name__ == '__main__':
    ...
    for objective in objectives:
        ...
        #for instance in ['basic', '15_more_games', '25_more_games', '']:
            ...
            for reschedule_mode in ['post_all_star']:
                for max_mods_per_tour in [2]:
                    for league in leagues:
                        ...
                        for n_window in [2]:
                            for asterisk in [0, 1]:
                                for max_non_dis_mods in [10]:
                                    for overlap_tours in [True]:
                                        if league == 'nba':
                                        ...
                                        if instance == 'basic':
                                            ...
                                            fixture = pd.read_csv(f'{base_cwd}/other_instances/nba_schedule_{instance}.csv')
                                        ...
                                        for s in range(len(starts)):
                                            ...
                                            print(f"Rescheduling matches for league {league} - objective {objective},  "
                                                    ...
                                                    f"- max mods per tour {max_mods_per_tour} - OverlapTours {overlap_tours}")
                                            ...
                                            M = TourSequenceModel(league, custom_fixture=fixture, start_date=start_date, end_date=end_date,
                                                                  disruptions=disruptions, non_disruptions=non_disruptions, 
                                                                  max_mods_per_tour=max_mods_per_tour, max_adj_days=n_window,
                                                            max_non_dis_mods=max_non_dis_mods, overlap_tours=overlap_tours)
                                            dis = M.disruptions
                                            
                                            # Create tour shifts
                                            shifts_dict, shifts_per_team_and_date_dict, shifts_per_tour, shifts_dict_tuples, shifts_per_team_and_date_dict_tuples, shifts_per_tour_tuples = M.get_tour_shifts_v2()
                                            # Insert
                                            shifts_dict, shifts_per_team_and_date_dict, shifts_per_tour, shifts_per_disruption, shifts_dict_tuples, shifts_per_team_and_date_dict_tuples, shifts_per_tour_tuples, shifts_per_disruption_tuples = M.insert_disruptions(shifts_dict, shifts_per_team_and_date_dict, shifts_per_tour,
                                                                                                                                                                                                                                                                      shifts_dict_tuples, shifts_per_team_and_date_dict_tuples, shifts_per_tour_tuples)
                                            
                                            print(len(shifts_dict))
                                            print("Building model!")
                                            prob_lp = cplex.Cplex()

                                            # We create the variables that will go into the model
                                            x_var_dict = M.create_decision_variables(shifts_dict_tuples)
                                            output_df, x_variables = M.solve_lp(x_var_dict, shifts_per_tour_tuples, shifts_per_team_and_date_dict_tuples, shifts_per_disruption_tuples, shifts_dict_tuples, prob_lp)
                                        ...
                                        if check == 1:
                                            ...
                                            fixture.to_csv(f'./output/BasicModel_{league}_{objective}_{distance_mode}_{instance}_{reschedule_mode}_{n_window}_{max_mods_per_tour}_{asterisk}_{feasibility_days}_{max_non_dis_mods}_{overlap_tours}Test.csv', index=False, encoding='utf-8 sig')
